...
var (
	...
	ErrStoreRollback     = errors.New("kauket: store.json version is older than the pinned store version")
	ErrRecoveryAuthority = errors.New("kauket: recovery key set change must be signed by a pinned recovery key")
)
